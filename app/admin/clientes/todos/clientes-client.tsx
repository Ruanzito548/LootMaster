...
export default function ClientesAdminClient() {
  ...
  const statCards: Array<{ icon: LucideIcon; label: string; value: number; caption: string; tone: string }> = [
    ...
    { icon: CircleHelp, label: "Sem parceiro", value: stats.withoutAgent, caption: `${rows.length ? ((stats.withoutAgent / rows.length) * 100).toFixed(1) : "0.0"}% do total`, tone: "text-[#f2b35f]" },
  ];
  ...
  if (!isAuthenticated) {
    return (
      <p className="mt-6 rounded-xl border border-amber-900 bg-amber-950/20 px-5 py-4 text-sm font-medium text-amber-300">
        Entre com uma conta de administrador para gerenciar clientes e parceiros.
      </p>
    );
  }
  ...
  return (
    <section className="mt-5 space-y-4">
      ...
      <div className="grid gap-3 rounded-xl border border-white/8 bg-[#101722] p-4 lg:grid-cols-[minmax(0,1.5fr)_repeat(3,minmax(0,1fr))_auto]">
        ...
        <label className="text-[0.58rem] font-bold uppercase tracking-[0.14em] text-[#8e98a3]">Parceiro<select value={agentFilter} onChange={(event) => setAgentFilter(event.target.value)} className="mt-2 w-full rounded-lg border border-white/10 bg-[#0c121b] px-3 py-2.5 text-sm text-[#e2e6ea]"><option value="all">Todos os parceiros</option><option value="none">Sem parceiro</option>{agents.map((agent) => <option key={agent.uid} value={agent.uid}>{agent.username}</option>)}</select></label>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-green-900 text-xs font-semibold uppercase tracking-wide text-green-600">
                ...
                <th className="px-4 py-3">Parceiro atual</th>
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <tr key={row.uid} className={`border-b border-green-950 ${index % 2 === 0 ? "" : "bg-green-950/20"}`}>
                    <td className="px-4 py-3">
                      ...
                      {row.isAgent ? (
                        <p className="mt-1 text-xs font-semibold text-emerald-400">
                          Parceiro ({row.agentFeeSharePercent.toFixed(2)}% da taxa da plataforma)
                        </p>
                      ) : null}
                    </td>
                    ...
                    <td className="px-4 py-3 text-xs text-[#e6c46a]">{row.assignedAgentId ?? "Sem parceiro"}</td>
                    <td className="px-4 py-3">
                      <div className="flex min-w-[420px] flex-wrap items-center gap-2">
                        ...
                        >
                          {loadingKey === promoteKey ? "Promovendo..." : row.isAgent ? "Já é parceiro" : "Tornar parceiro"}
                        </button>
                        ...
                        >
                          {loadingKey === unassignKey ? "Desvinculando..." : "Desvincular parceiro"}
                        </button>
                      </div>
                    </td>
                  </tr>
