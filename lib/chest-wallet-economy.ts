  ...
  const ledger = Array.isArray(parsed.ledger)
    ? parsed.ledger.map((entry) => {
        ...
        return {
          ...
          description,
          createdAt,
          ...(raw.orderId ? { orderId: String(raw.orderId) } : {}),
          ...(raw.rewardId ? { rewardId: String(raw.rewardId) } : {}),
          ...(raw.userId ? { userId: String(raw.userId) } : {}),
          ...(raw.source ? { source: String(raw.source) } : {}),
          ...(raw.referenceId ? { referenceId: String(raw.referenceId) } : {}),
          ...(raw.createdAtMs ? { createdAtMs: Number(raw.createdAtMs) } : {}),
          ...(typeof raw.metadata === "object" && raw.metadata ? { metadata: raw.metadata as Record<string, unknown> } : {}),
...
export function applyChestWalletReward(
  ...
  nextState.ledger.push({
    ...
    metadata: {
      ...
      ...(context?.chestId ? { chestId: context.chestId } : {}),
